...
try {
  ...
  fs.writeFileSync(runScript, [
    ...
    `const cacheBase = join(require('os').homedir(), '.claude', 'plugins', 'cache', 'deepseek-monitor');`,
    `let hudPath = '';`,
    ...
    "try { writeFileSync(join(require('os').homedir(), '.claude', 'deepseek-cache', 'hud.pid'), String(process.pid)); } catch {}",
    "hud.main();",
    "",
  ].join('\n'));
